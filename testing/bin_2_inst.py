...
def main():
    ...
    with open('testing/inst.txt') as f:
        lines = f.readlines()
        f.close()
    bin_str_pair = {'11111111111111111111111111111111' : 'finish'}
    for inst in lines:
        inst_binary = convert_to_binary(inst)
        # print(inst + ' : ' +inst_binary)
        bin_str_pair[inst_binary] = inst.replace('\n', '')

    with open("testing/exe_inst.txt", 'r') as f:
        exe_inst = f.readlines()
        f.close()

    print(f'line num = {len(exe_inst)}')
    
    with open("testing/exe_inst_str.txt", 'w') as f:
        for lines in exe_inst:
            lines = lines.replace('\n', '')
            f.write(bin_str_pair[lines])
            f.write('\n')
        f.close()
